...
BEGIN
	...
	AS
	(
		SELECT [row] = ROW_NUMBER() OVER(PARTITION BY [n].[data].[value]('(//blocked-process/process/@id)[1]','NVARCHAR(128)') ORDER BY [n].[data].[value]('(@timestamp)[1]','DATETIME2') DESC)
			,[timestamp_utc] = [n].[data].[value]('(@timestamp)[1]','DATETIME2')
