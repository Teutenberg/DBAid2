	...
	SELECT 'HADR' AS [heading], 'Replication' AS [subheading], '**Publishers**' AS [comment]
	...
	INSERT INTO @db_publication EXEC sp_MSforeachdb  N'SELECT ''?'' FROM [?].[INFORMATION_SCHEMA].[TABLES] [T] INNER JOIN [sys].[databases] [D] ON ''?'' = [D].[name] WHERE [TABLE_NAME]=''syspublications''  AND [D].[is_distributor]=0';
